...
class EncoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        modules.append(nn.Conv2d(channels[-1], out_channels, kernel_size=conv_ker_size, stride=stride, padding = padding))
...
class VAE(nn.Module):
    def __init__(self, features_encoder, features_decoder, in_size, z_dim):
        ...
        # ====== YOUR CODE: ======
        self.log_sig_layer = nn.Linear(n_features, z_dim)
        self.mu_layer = nn.Linear(n_features, z_dim)

        self.decode_layer = nn.Linear(z_dim, n_features)
    ...
    def encode(self, x):
        ...
        mu = self.mu_layer(x)
        log_sigma2 = self.log_sig_layer(x)
        z = mu + (log_sigma2.mul(.5).exp_()) * torch.randn_like(mu)
        # ========================
    ...
    def decode(self, z):
        ...
        # ====== YOUR CODE: ======
        reconstructed_in = self.decode_layer(z)
        x_rec = self.features_decoder(reconstructed_in.view([-1, *self.features_shape]))
        #x_rec = reconstructed_in
        # ========================
...
def vae_loss(x, xr, z_mu, z_log_sigma2, x_sigma2):
    ...
    # ====== YOUR CODE: ======
    N = x.shape[0]
    dx = (x_sigma2 * N * x.shape[1] * x.shape[2] * x.shape[3])
    data_loss = ((x - xr).norm() ** 2 / dx)
    kldiv_loss = (z_log_sigma2.exp().sum() + z_mu.norm() ** 2 - z_log_sigma2.sum() - z_mu.shape[1] * N) / N
