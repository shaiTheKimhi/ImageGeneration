...
class RNNTrainer(Trainer):
    ...
    def train_epoch(self, dl_train: DataLoader, **kw):
        ...
        h_size = (dl_train.batch_size, self.model.n_layers, self.model.h_dim)
        self.h = torch.zeros(h_size, requires_grad=True, device=self.device)
        return super().train_epoch(dl_train, **kw)

    def test_epoch(self, dl_test: DataLoader, **kw):
       
        h_size = (dl_test.batch_size, self.model.n_layers, self.model.h_dim)
        self.h = torch.zeros(h_size, device=self.device)
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        self.h.detach_()
        pred_prob, self.h = self.model(x, self.h)
        pred = torch.argmax(pred_prob, 2)
        loss = self.loss_fn(pred_prob.transpose(1, 2), y) 
        loss.backward()
        self.optimizer.step()
...
class VAETrainer(Trainer):
    def train_batch(self, batch) -> BatchResult:
        ...
        xr, z_mu, log_sigma2 = self.model(x)
        loss, data_loss, _ = self.loss_fn(x, xr, z_mu, log_sigma2)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            xr, z_mu, log_sigma2 = self.model.forward(x)
            loss, data_loss, _ = self.loss_fn(x, xr, z_mu, log_sigma2)
